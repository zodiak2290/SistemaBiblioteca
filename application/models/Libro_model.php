...
Class Libro_model extends CI_Model
{
    ...
    {
        $this->db->select(
            "DATE_ADD(fechaprestamo,interval (diasentrega+(renovacion*contreno) )  day)  as entregar
          ,(case  
  ...
  a realizado la devolución te sugerimos reserva otro ejemplar.'
    ...
    private function recomendapormaterias($materias,$nadqui="")
    {
        ...
        if($nadqui) {
            ...
            $query=$this->db->query(
                "select ej.created_at,titulo,ej.nadqui as nad,nameautor,clasificacion
...
join autor on autor.idautor=ae.autor_id 
    ...
    {
        $query=$this->db->query(
            "select (case  when clasificacion rlike '^[0][0-9][0-9]' then 'Generalidades'
