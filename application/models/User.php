...
Class User extends CI_Model
{
 ...
 function find_by_cuenta($cuenta){
    $query=$this->db->query("select pnombre,apPat,apMat,cantlibros,diasentrega,renovacion,montomulta 
 ...
 function uservigente($cuenta){
     $query=$this->db->query("select * FROM personas join usuariobiblio on personas.curp=usuariobiblio.pcurp
...
function get_datos_prestamos_usuarui($cuenta,$tipo){
 ...
 function is_userbiblio($cuenta){
  ...
  function multas($cuenta){
    $query=$this->db->query(" select idmulta, fechamulta,datediff(fechadev,fechaprestamo)-diasentrega as retraso, (datediff(fechadev,fechaprestamo)-diasentrega) * montomulta as total
    FROM prestamos as p 
  ...
  function total_multa_saldo($cuenta){
    $query=$this->db->query("select count(*) as total ,sum(pagado) as monto
                    FROM multas 
  ...
  function total_multa_pendientes_saldo($cuenta){
    ...
    function nopermitirreserva($cuenta){
      ...
      function preferenciasusuario($cuenta,$tipo){
          $query=$this->db->query("select (".$this->caseclasificacion.") 
