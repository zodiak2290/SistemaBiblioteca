...
class VerifyLogin extends MY_Controller {
   ...
   function index()
   {
       ...
       $this->form_validation->set_rules('cuentausuario','Cuenta','required');
       $this->form_validation->set_rules('password_d','Password','required');
   ...
   private function get_credenciales(){
      //Obtenemos User y pass enviados por post
      $cuenta = $this->input->post('cuentausuario');
      $password = $this->input->post('password_d');
      $this->check_database($cuenta,$password);
   }  
   ...
   function validar_Intentos($intentos_fecha){
      $num_intentos=0;
   ...
   function check_database($cuenta,$password){  
      $result=false;
      $bloqueo=false;
      ...
      $datos=[];
      $acccesotipo="";
      if(($this->User->is_userbiblio($cuenta)&&is_numeric($cuenta))||$this->Persona_model->findaemail($cuenta)){
